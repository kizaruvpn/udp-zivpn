...
echo "⏰ Setting auto backup (cron)..."
CRON_JOB="0 * * * * /usr/local/bin/zivpn_helper.sh backup >> /var/log/zivpn_backup.log 2>&1"
# pastikan executable
chmod +x /usr/local/bin/zivpn_helper.sh
# tambah cron kalau belum ada
(crontab -l 2>/dev/null | grep -v "zivpn_helper.sh backup"; echo "$CRON_JOB") | crontab -
